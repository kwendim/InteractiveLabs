...
def index(request):
    ...
    with open(join(path, 'hooks/config.json'), 'r') as cfg:
    ...
    if secret:
        ...
        # Python does not havpathe hmac.compare_digest prior to 2.7.7
    ...
    logging.info('Metadata:\n{}'.format(dumps(meta))) #TODO remove logging?
    print('Metadata:\n{}'.format(dumps(meta)))
    # Skip push-delete
    if 'deleted' in payload and event == 'push':
    ...
    # with fdopen(osfd, 'w') as pf: # initial code
    ...
    output = JsonResponse(ran)
    logging.info("final output{}".format(output))
    return output
